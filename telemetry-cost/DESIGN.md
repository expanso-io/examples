...
```
README.md LICENSE DEMO.md RESULTS.md DESIGN.md justfile docker-compose.yml presets.json .gitignore
...
`just edge` (start local edge), `just board` (start costboard), `just generator`,
`just demo1..demo4`, `just demo3-step1..4`, `just audit`, `just rehydrate <from> <to>`,
`just test`, `just eval`, `just reset`, `just clean` (kills edge+board, removes
.edge-data*, data/, cold-storage/). Demos deploy via expanso-cli against 19010.
Every recipe idempotent; `just clean` MUST leave no processes (pkill by exact
...
Fixtures: `just fixtures` regenerates `fixtures/{app.ndjson,k8s.log,web.log,cloudtrail.ndjson}`
...
## Cloud-first orchestration (docs + justfile)
...
<network> --api-key ... --select`, then `EDGE_FLAGS="--profile demo" just demo1`.
justfile: DEPLOY uses `$(DEPLOY_FLAGS)` (default `--endpoint $(EDGE_ENDPOINT)`,
overridable with `--profile <name>`). Local mode stays as the offline/CI path.
just sims / sims-stop: launch/stop the logsim tcp streams (pidfiles in .run/).
...
jobs/cloud/*.yaml        # selector-injected variants (generated by `just cloud-jobs`)
...
8. Write .demo-cloud.env: DEMO_PROFILE="$PROFILE". Print "Setup done -> run: just demo".
...
2. `just cloud-jobs` (regenerate jobs/cloud/ from base + selector). board up (local costboard).
...
4. Per scenario (guided; pause for Enter unless --no-pause):
   ...
   - tiers scenario: after collecting, run `just rehydrate <from> <to>` for the
     auditor beat.
...
## justfile (cloud-first)

- `just demo` -> scripts/demo.sh (cloud). `just demo --scenario filter` passes through.
- `just cloud-setup` -> scripts/cloud_setup.sh. `just doctor` -> scripts/preflight.sh.
- `just demo-local` -> scripts/demo.sh --local (the offline path; what old `just demo1..4` were).
- `just cloud-jobs` -> generate jobs/cloud/*.yaml: copy base job, inject
...
- `just doctor` runs and correctly reports red when no demo profile exists.
